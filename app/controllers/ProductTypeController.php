...
class ProductTypeController
{
    ...
    public function __construct() {
        ...
        $this->productType = new ProductType($conn);
    ...
    { 
        if ($this->auth->validateCredentials()) {
            $response = $this->productType->list();
            if (count($response) == 0) {
    ...
    { 
        if ($this->auth->validateCredentials()) {
            $response = $this->productType->read($id);
            if ($response === false) {
                header('HTTP/1.0 200 OK');
    ...
    public function create()
    {
        if ($this->auth->validateCredentials()) {
            ...
            $response = $this->productType->create($data['name'], $data['description'], $data['sector'], $data['id_tx']);

            if ($response['rowCount'] > 0) {
                header('HTTP/1.0 200 OK');
                $response = "ProductType ". $data['name'] . " [".$data['description'] ."] successfully created";
                $response = array('code' => 200, 'response' => $response);
                echo $response = json_encode($response);
            }
            else {
    ...
    public function update()
    {
        if ($this->auth->validateCredentials()) {
            ...
            $data = json_decode(file_get_contents('php://input'), true);
            $response = $this->productType->update($data['id'], $data['name'], $data['description'], $data['sector'], $data['id_tx']);

            if ($response['rowCount'] > 0) {
                header('HTTP/1.0 200 OK');
                $response = ("ProductType ". $data['name'] . " [".$data['description'] ."] updated successfully");
                $response = array('code' => 200, 'response' => $response);
                echo $response = json_encode($response);
            }
            else {
    ...
    public function delete()
    {
        if ($this->auth->validateCredentials()) {
            $data = json_decode(file_get_contents('php://input'), true);
            $response = $this->productType->delete($data['id']);
